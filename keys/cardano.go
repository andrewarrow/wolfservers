...
func StakePoolRegCert(ada int64, margin int, hash, code string) {
	cmd := "cardano-cli"
	lower := strings.ToLower(code)
	tokens := ToTokens(fmt.Sprintf("stake-pool registration-certificate --cold-verification-key-file node.vkey --vrf-verification-key-file vrf.vkey --pool-pledge %d000000 --pool-cost 340000000 --pool-margin 0.%02d --pool-reward-account-verification-key-file stake.vkey --pool-owner-stake-verification-key-file stake.vkey --mainnet --single-host-pool-relay relay1-%s.wolfschedule.com --pool-relay-port 6000 --metadata-url https://wolfschedule.com/assets/%s.json --metadata-hash %s --out-file pool.cert", ada, margin, lower, code, hash))
